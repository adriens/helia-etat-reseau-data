...
function toIcalDt(iso: string): string {
  // ISO 8601 with offset → UTC → iCal YYYYMMDDTHHMMSSZ
...
export async function GET() {
  ...
  const cal = [
    ...
    'CALSCALE:GREGORIAN',
    'METHOD:PUBLISH',
    fold(`X-WR-CALNAME:Maintenances Helia NC`),
    'X-WR-TIMEZONE:Pacific/Noumea',
    fold(`X-WR-CALDESC:Maintenances programmées réseau Helia by OPT-NC`),
    events,
    'END:VCALENDAR',
  ].join('\r\n');
